...
# This program requires all tab of the internet browser to be closed while running.
...
def get_browserhistory() -> dict:
    ...
    for browser, path in paths2databases.items():
        try:
            ...
            if browser == 'chrome':
                _SQL = """SELECT url FROM urls ORDER BY last_visit_time DESC"""
            elif browser == 'firefox':
                _SQL = """SELECT url FROM moz_places INNER JOIN moz_historyvisits on moz_historyvisits.place_id = moz_places.id ORDER BY visit_date DESC"""
            elif browser == 'safari':
                _SQL = """SELECT url FROM history_visits INNER JOIN history_items ON history_items.id = history_visits.history_item ORDER BY visit_time DESC"""
            else:
                pass
...
bb=dict(zip(list(history),[list(history).count(i) for i in list(history)]))
pp.pprint(bb)
